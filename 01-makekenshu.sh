...
msgoutput(){
    ...
    echo $datetemp $1 >> ./result.txt
}

msgoutput "[Inf]研修環境構築スクリプト Ver0.1"
...
#ホストOS更新処理
msgoutput "[Inf]ホストOSのアップデートを行います。"
yum update -y
msgoutput "[Inf]ホストOSのアップグレードを行います。"
yum upgrade -y

#Dockerインストール
msgoutput "[Inf]Dockerをインストールします。"
...
if [ $? -eq 0 ]; then
    ...
    docker rm webapp
    if [ $? -gt 0 ]; then
        msgoutput "[Inf][webapp]の削除が失敗したため、強制削除を実施します。"
...
if [ $? -gt 0 ]; then
    msgoutput "[Inf]Dockerイメージ[dockerfiles/django-uwsgi-nginx]をダウンロードします。"
    ...
    msgoutput "[Inf]Dockerイメージ[dockerfiles/django-uwsgi-nginx]はすでに存在します。"
fi
...
if [ $? -eq 0 ]; then
    msgoutput "[Suc]セットアップが成功しました。"
else
    msgoutput "[Err]セットアップが失敗しました。"
    exit
fi
...
docker ps | grep webapp
if [ $? -eq 0 ]; then
    msgoutput "[Suc]本セットアップが成功しました。"
else
    msgoutput "[Suc]本セットアップが失敗しました。"
    exit
fi
...
msgoutput "[Inf]Dockerコンテナ[webapp]への[sed]のインストールを開始します。"
#docker exec -it webapp apt-get install -y sed
if [ $? -eq 0 ]; then
...
#Dockerコンテン内のDjangoプロジェクトをシェルの有効化
msgoutput "[Inf]コンテナ内に02-remakeproject.shを格納します。"
docker cp ./02-remakeproject.sh webapp:/home/docker/code/
if [ $? -eq 0 ]; then
...
docker exec -it webapp chmod 777 /home/docker/code/02-remakeproject.sh
if [ $? -eq 0 ]; then
...
docker exec -it -d webapp /home/docker/code/02-remakeproject.sh
if [ $? -eq 0 ]; then
...
#result.txt結合
cat ./code/result.txt >> ./result.txt

msgoutput "[Inf]uwsgi.iniをコンテナ内にコピーします。"
\cp -f ./03-uwsgi.ini ./code/uwsgi.ini
if [ $? -eq 0 ]; then
