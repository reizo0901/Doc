...
if [ $? -gt 0 ]; then
    ...
    if [ $? -gt 0 ]; then
        ...
        echo `date '+%y/%m/%d %H:%M:%S'` "[Success]インストールが成功しました。" >> ./result.txt
    else
...
if [ -e "/home/ec2-user/docker/code" ]; then
    ...
    if [ -e "/home/ec2-user/docker/code" ]; then
        echo "[Success]削除が成功しました。"
        echo `date '+%y/%m/%d %H:%M:%S'` "[Success]削除が成功しました。" >> ./result.txt
    else
        echo "[Error]削除が失敗しました。"
        echo `date '+%y/%m/%d %H:%M:%S'` "[Error]削除が失敗しました。" >> ./result.txt
        exit
    fi
else
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]過去の構築ディレクトリは存在しませんでした。" >> ./result.txt
fi
...
if [ $? -gt 0 ]; then
    ...
    if [ $? -gt 0 ]; then
        echo "ダウンロードが成功しました。"
        echo `date '+%y/%m/%d %H:%M:%S'` "[Success]ダウンロードが成功しました。"
    else
        echo "ダウンロードが失敗しました。"
...
if [ $? -eq 0 ]; then
    echo "セットアップが成功しました。"
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]セットアップが成功しました。" >> ./result.txt
else
    echo "セットアップが失敗しました。"
...
if [ $? -eq 0 ]; then
    echo "環境コピーが成功しました。"
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]環境コピーが成功しました。" >> ./result.txt
else
    echo "環境コピーが失敗しました。"
...
echo `date '+%y/%m/%d %H:%M:%S'` "[Success]Dockerコンテナの本セットアップを開始します。" >> ./result.txt
docker run -it -d -p 80:80 --name webapp -v /home/ec2-user/docker/code:/home/docker/code dockerfiles/django-uwsgi-nginx
docker ps | grep webapp
if [ $? -eq 0 ]; then
    echo "本セットアップが成功しました。"
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]本セットアップが成功しました。" >> ./result.txt
else
    echo "本セットアップが失敗しました。"
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]本セットアップが失敗しました。" >> ./result.txt
    exit
fi
...
if [ $? -eq 0 ]; then
    ...
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success][Vim]のインストールに成功しました。" >> ./result.txt
else
...
echo "settings.pyをリネームします。"
mv /home/ec2-user/docker/code/app/website/settings.py /home/ec2-user/docker/code/app/website/settings_tmp.py
if [ $? -eq 0 ]; then
    echo "[Success]リネームが成功しました。"
else
    echo "[Error]リネームが失敗しました。"
    exit
fi
echo "コンテナ内のDjangoプロジェクトファイル[settings_tmp.py]を修正します。"
...
if [ $? -eq 0 ]; then
    echo "コメントアウトに成功しました。"
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]コメントアウトに成功しました。" >> ./result.txt
else
    echo "コメントアウトに失敗しました。"
...
if [ $? -eq 0 ]; then
    echo "追記に成功しました。"
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]追記に成功しました。" >> ./result.txt
else
    echo "追記に失敗しました。"
...
if [ $? -eq 0 ]; then
    echo "再起動が成功しました。"
    echo `date '+%y/%m/%d %H:%M:%S'` "[Success]再起動が成功しました。" >> ./result.txt
