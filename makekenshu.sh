...
if [ $? -gt 0 ]; then
    ...
    echo "Dockerを起動します。"
    echo `date '+%y/%m/%d %H:%M:%S'` "Dockerを起動します。" >> ./result.txt
    systemctl start docker
    echo "Dockerを永続化します。"
    echo `date '+%y/%m/%d %H:%M:%S'` "Dockerを永続化します。" >> ./result.txt
else
...
echo `date '+%y/%m/%d %H:%M:%S'` "コンテナ内のDjangoプロジェクトファイル[settngs.py]を修正します。" >> result.txt
...
if [ $? -eq 0 ]; then
    echo "ALLOWED_HOSTSを追記に成功成功しました。"
    echo "ALLOWED_HOSTSを追記に成功成功しました。"
else
    echo "ALLOWED_HOSTSの追記に失敗しました。"
    echo "ALLOWED_HOSTSの追記に失敗しました。"
fi
